...
.設問
* `javac` コマンドは、Javaソースコードを中間コードに変換する。[コンパイラ]
* あるプログラミング言語のソースコードを他のプログラミング言語のソースコードに変換する。[トランスパイラ]
* 関数の引数や演算の引数が妥当な型なのかチェックする。[意味解析]
* 式や文に構成する際に、トークンの並びから作成するもの。[構文木]
* 終端記号・非終端記号・出発記号・生成規則によってプログラミング言語の構造を表す。[文脈自由文法]
* ソースコードを実行可能なコードに変換する（実行はしない）。[インタプリタ]
* Loxの式や文をを処理するとき利用した処理技法。[Visitor]
* プログラム中で利用している変数が定義されているかチェックする。[構文規則]
* 文脈自由文法の生成規則を表現するのに利用する簡便な記法。[BNF]
...
【問4】cloneできたこと。その画像ファイルで、下記ファイル（ `images/cloning_lox01.jpg` ）を置き換えなさい。
...
** `https://classroom.github.com/a/HweuLHTG`
** `https://classroom.github.com/a/C9LraX0c`
